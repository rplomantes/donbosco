...
<div class="container">
  ...
  <div id="searchbody">   
            ...
            <tr><th>Student Number</th><th>Student Name</th><th>Gender</th><th>Student Info</th><th>Student Grade</th></tr>        
            </thead>
            <tbody>
               
            @foreach($students as $student)
            <tr><td>{{$student->idno}}</td><td>{{$student->lastname}}, {{$student->firstname}} {{$student->middlename}}
                    {{$student->extensionname}}</td><td>{{$student->gender}}</td><td><a href = "{{url('/studentinfokto12',$student->idno)}}" target="_blank">View Info</a></td><td><a href="{{url('seegrade',$student->idno)}}">View Grade</a></td></tr>
            @endforeach
            </tbody>
            </table>
                 </div>
