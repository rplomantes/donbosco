...
<form method='POST' id="subForm" target='_blank'>
    ...
    <hr>
    <?php
    $acctGroup = $viewaccounts->groupBy('acctdepartment');
    ?>
    ...
    <div class='panel panel-primary'>
        <div class='panel-heading'>
            <div class="panel-title">
                <span>
                    <b>{{$accts->pluck('acctdepartment')->last()}}</b>
                </span>
                <span style="pull:right">
                    <button class='btn btn-danger' onclick="postform('{{$accts->pluck('acctdepartment')->last()}}')">Print</button>
                    <button class='btn btn-success' onclick="downloadform('{{$accts->pluck('acctdepartment')->last()}}')">Download</button>
                </span>
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            <table class="table table-striped">
                <tr>
                    <th class='date' width="8%"></th>
                    <th class='refno' width="10%"></th>
                    <th class='name' width="20%"></th>
                    <th class='level'></th>
                    <th class='section'></th>
                    <th class='debit'>Debit</th>
                    <th class='credit'>Credit</th>
...
<script>
    ...
    function postform(subsidiary){
        ...
        forms.action = '{{url("/summary/individualsubaccount/printdepartment")}}/'+subsidiary;
    ...
    function downloadform(subsidiary){
        forms = document.getElementById('subForm');
        forms.action = '{{url("/summary/individualsubaccount/downloaddepartment")}}/'+subsidiary;
