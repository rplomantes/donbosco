...
<div class="container">
  ...
  <div id="searchbody">   
            ...
            <tr><td>{{$student->idno}}</td><td>{{$student->lastname}}, {{$student->firstname}} {{$student->middlename}}
                    {{$student->extensionname}}</td><td>{{$student->gender}}</td><td><a href = "{{url('/studentinfo',$student->idno)}}">view</a></td><td><a href = "{{url('/ledger',$student->idno)}}">view</a></td></tr>
            @endforeach
            </tbody>
            </table>
                 </div>
